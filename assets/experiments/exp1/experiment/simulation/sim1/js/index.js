...
function setupEvents() {
  ...
  el.addEventListener('touchstart', e => {
    if (e.touches.length === 1) {
      ...
      el._pinchDist = Math.hypot(dx, dy);
      el._camDist0 = camDist;
      mDown = false; 
    }
  }, { passive: false });
  ...
  el.addEventListener('touchmove', e => {
    ...
    if (e.touches.length === 1 && mDown) {
      tRotX = tX0 + (e.touches[0].clientX - mX0) * 0.005;
      tRotY = tY0 + (e.touches[0].clientY - mY0) * 0.005;
      updateCamera();
    } else if (e.touches.length === 2) {
      const dx = e.touches[0].clientX - e.touches[1].clientX;
      const dy = e.touches[0].clientY - e.touches[1].clientY;
      ...
      if (el._pinchDist > 0) {
        const ratio = el._pinchDist / dist;
        camDist = Math.max(6, Math.min(100, el._camDist0 * ratio));
        updateCamera();
      }
    }
  }, { passive: false });

  el.addEventListener('touchend', e => { mDown = false; });
