...
if (has3D) {
  ...
  scene.fog = new THREE.FogExp2(COL.bgNight, 0.025);
  ...
  scene.add(new THREE.AmbientLight(0x0f1b35, 1.0));
  const consoleSpot = new THREE.DirectionalLight(0xdff1ff, 1.3);
  consoleSpot.position.set(5, 12, 8);
  consoleSpot.castShadow = true;
  consoleSpot.shadow.bias = -0.0005;
  scene.add(consoleSpot);

  const indicatorGlow = new THREE.PointLight(COL.cyan, 1.2, 10);
...
tabBtn1.onclick = () => {
  ...
  if (gridLayout) gridLayout.style.gridTemplateColumns = '1.2fr 1fr';
